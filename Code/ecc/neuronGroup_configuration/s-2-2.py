...
N_astro = 7 # Total number of astrocytes in the network
astrocytes = NeuronGroup(N_astro, astro_eqs, method='rk4')
astrocytes.I_bias = np.asarray([1, 0., 0.,0.,0.,0.,0.],dtype=float)*umolar
...
astro_to_astro.connect(i=1,j=3)
#branch 2
astro_to_astro.connect(i=0,j=4)
astro_to_astro.connect(i=4,j=5)
astro_to_astro.connect(i=4,j=6)
...
#plt.savefig("plot_s-2-2_input.pdf")
plt.show()
