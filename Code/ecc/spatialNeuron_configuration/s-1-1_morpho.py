...
morpho = Soma(diameter=25*um)
morpho.c1 = Cylinder(length=.47*um, diameter=25*um,n=1)
morpho.c2 = Cylinder(length=.47*um, diameter=25*um,n=1)

astrocyte = SpatialNeuron(morphology=morpho, model=astro_eqs, method='rk4')
#astrocyte = NeuronGroup(N_astro, astro_eqs, method='rk4')

astrocyte.I_bias = np.asarray([1, 0., 0.],dtype=float)*umolar
astro_mon = StateMonitor(astrocyte, variables=['C', 'J_chan', 'J_leak', 'J_pump', 'h',
