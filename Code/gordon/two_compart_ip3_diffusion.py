...
sim_dt = 1.*ms               # Integrator/sampling step (too small!!)  (beyond 0.30*ms, there is 2-point oscillation in solution)
...
Jbeta=coef * 0.1e-24*mole/second   # IP3 input
...
# --- IP3 diffusion
D_IP3 = 1.e-8*Hz     # TEMPORARY value
# --- IP_3R kinectics
...
comp_eqs = '''
dIP3/dt=(Jbeta+Jdelta-J3k-J5p-Jdiff_IP3)*1e3/(Lambda*(1-rho))  : mmolar
...
J3k=coef*O_3K * ( ( CaCYT**4/(CaCYT**4+K_D**4)) * (IP3/(K_3K+IP3) ) ) : mole/second
J5p=coef*O_5P * IP3 /(IP3 + K_5P) : mole/second
...
compartments.IP3 = [0.000,0.0]*umolar   # Units: umolar or ymolar? 
#compartments.IP3 = [0.101,0.0]*umolar   # Units: umolar or ymolar?  orig
...
astro_diff_mon = StateMonitor(compartments, variables=['Jdiff_IP3'], record=True)
...
#print(astro_diff_mon.Jdiff_IP3)
print("CaCYT, ", astro_Ca_mon.CaCYT)
print("IP3, ", astro_IP3_mon.IP3)
for j in range(2):
  for i in range(10):
   print("Jdiff_IP3, ", i, astro_diff_mon.Jdiff_IP3[j][i])
quit()
################################################################################
